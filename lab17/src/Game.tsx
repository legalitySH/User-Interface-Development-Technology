...
const Game: React.FC = () => {
  ...
  const checkValidity = (matrix: (number | null)[][], readOnlyMatrix: boolean[][], lastChangeCell: [number, number] | null) : void => {
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
  ...
  return (
      <>
        ...
        <div className="buttonsContainer">
          ...
          <button className="btn" onClick={handleCheckValidity}>
            Проверить
          </button>
        </div>
      </>
